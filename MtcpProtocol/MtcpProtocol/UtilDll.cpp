...
std::string UtilDll::GetWorkDirectory()
{
    std::string path = "";
#ifdef _WINDOWS
    char file_path[1024];
...
std::string UtilDll::GetPathIncludeSlash(const std::string& fullpath)
{
...
std::string UtilDll::GetPathUnIncludeSlash(const std::string& fullpath)
{
    if (fullpath.length() == 0)
        return fullpath;
    std::string path = fullpath;
...
std::string UtilDll::GetProcessName()
{
    std::string proName = "";
#if defined(_WINDOWS)
    char file_path[1024];
...
std::string UtilDll::GetFileExt(const char* path)
{
...
char* UtilDll::basename(const char* path)
{
    const char* s;
    const char* p;
    char c;
...
void UtilDll::Split(const std::string& src, const std::string& split, std::vector<std::string>& results)
{
    std::string tSrc = src;
    while (tSrc.length() > 0) {
...
std::wstring UtilDll::StringToWString(const std::string& str)
{
...
std::string UtilDll::WStringToString(const std::wstring& wstr)
{
    std::string curLocale = setlocale(LC_ALL, NULL);  // curLocale = "C";
...
std::string UtilDll::UnicodeToUTF8(const std::wstring& wstr)
{
    std::string ret;
    try {
...
std::wstring UtilDll::UTF8ToUnicode(const std::string& str)
{
    std::wstring ret;
    try {
        std::wstring_convert<std::codecvt_utf8<wchar_t>> wcv;
...
std::string UtilDll::UnicodeToANSI(const std::wstring& wstr)
{
    return WStringToString(wstr);
}

std::wstring UtilDll::ANSIToUnicode(const std::string& str)
{
    return StringToWString(str);
}

std::string UtilDll::UTF8ToANSI(const std::string& str)
{
    return UnicodeToANSI(UTF8ToUnicode(str));
}

std::string UtilDll::ANSIToUTF8(const std::string& str)
{
    return UnicodeToUTF8(ANSIToUnicode(str));
}

// mode:替换模式，0：只替换第一个找到的。1：全部替换找到的。
int UtilDll::Replace(std::string& str, const std::string& pattern, const std::string& newpat, int mode)
{
    int count = 0;
...
void UtilDll::ReplaceToESC(std::string& str)
{
    Replace(str, "\\0", "\0", 1);
    Replace(str, "\\n", "\n", 1);
    Replace(str, "\\r", "\r", 1);
...
int UtilDll::MakeNDir(const std::string& fullPath)
{
    std::string path = fullPath;
    Replace(path, "\\", "/", 1);
...
bool UtilDll::IsFileExist(const std::string& fullPath)
{
#ifdef _WINDOWS
...
bool UtilDll::IsDirExist(const std::string& fullPath)
{
#ifdef _WINDOWS
    if (-1 == _access(fullPath.c_str(), 0))
        return false;
...
std::string UtilDll::GetFileName(const std::string& fullPath)
{
    std::string path = fullPath;
...
std::string UtilDll::GetFileNameWithOutExt(const std::string& fullPath)
{
...
std::string UtilDll::GetFilePath(const std::string& fullPath)
{
    std::string path = fullPath;
    std::string na = fullPath;
    Replace(path, "\\", "/", 1);
...
int UtilDll::GetPidByPort(int port)
{
#ifdef _WINDOWS
    std::string output;
    char szCommand[1024];
...
bool UtilDll::KillProcess(int pid)
{
#ifdef _WINDOWS
...
int UtilDll::GetFileLength(FILE* fp)
{
...
int UtilDll::ExecCommand(const std::string& cmd, std::string& output)
{
...
std::string UtilDll::Format(const char* format, ...)
{
    char szBuf[8192];
    va_list arglist;
...
std::vector<unsigned long> UtilDll::KillProcessByName(const std::string& procName)
{
    ...
    system(szCommand);
#else
    pidList = UtilDll::GetProcessByName(procName);
    for (unsigned long pid : pidList) {
		UtilDll::KillProcess(pid);
    }
...
std::vector<unsigned long> UtilDll::GetProcessByName(const std::string& procName)
{
    std::vector<unsigned long> pidList;
#if defined(_WINDOWS)
...
void UtilDll::SleepForMilliSeconds(int milliseconds)
{
#ifdef _WINDOWS
    Sleep(milliseconds);
#else
...
void UtilDll::Trim(std::string& src, const char* chs, size_t len)
{
    if (chs == NULL) {
        return;
    }
...
void UtilDll::KillThread(std::thread* thread)
{
#ifdef _WINDOWS
...
std::string UtilDll::GetIconFullPathName(const std::string& icon)
{
    std::string fullpathname = UtilDll::GetWorkDirectory() + "/" + icon;
    if (UtilDll::IsFileExist(fullpathname))
        fullpathname = "";
    return fullpathname;
}

bool UtilDll::StrCompare(const std::string& str1, const std::string& str2)
{
    std::string s1, s2;
    s1 = str1;
    s2 = str2;
...
std::string UtilDll::strToHex(const std::string& convert_data)
{
    std::string outStr;
...
std::string UtilDll::hexToStr(const std::string& convert_data)
{
    std::string outStr;
    char buf[6];
