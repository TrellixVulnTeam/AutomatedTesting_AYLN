...
std::string cLogHandle::g_path = "C:\\vault\\Intelli_Log\\MTCProto_Log";
...
int cLogHandle::LogMsg(std::string str, bool timeStamp)
{
	...
	if (timeStamp == true)
	{
		...
		SYSTEMTIME time;
		GetLocalTime(&time);
		sprintf_s(d, "%u-%u-%u_%u:%u:%u.%u\t", time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond, time.wMilliseconds);
		tmp += std::string(d);
	}
	tmp += str;
	//tmp += std::string("\r\n");
