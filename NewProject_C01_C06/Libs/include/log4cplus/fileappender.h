...
namespace log4cplus
{
	...
	class LOG4CPLUS_EXPORT DailyRollingFileAppenderEx : public DailyRollingFileAppender {
	public:
		// Ctors
		DailyRollingFileAppenderEx(const log4cplus::tstring& filename,
			DailyRollingFileSchedule schedule = DAILY,
			bool immediateFlush = true,
			int maxBackupIndex = 10,
			bool createDirs = false,
		...
		// Dtor
		virtual ~DailyRollingFileAppenderEx();
	...
	private:
		log4cplus::tstring CrreateFileName(log4cplus::tstring nam, DailyRollingFileSchedule schedule);
		LOG4CPLUS_PRIVATE void init(DailyRollingFileSchedule schedule);

		log4cplus::tstring srcName;
	};
