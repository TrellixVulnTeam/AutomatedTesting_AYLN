...
std::string Util::GetPathIncludeSlash(const std::string& fullpath)
{
...
std::string Util::GetPathUnIncludeSlash(const std::string& fullpath)
{
    if (fullpath.length() == 0)
        return fullpath;
    std::string path = fullpath;
...
std::string Util::GetFileExt(const char* path)
{
...
void Util::GetFiles(const std::string& path, std::vector<std::string>& files, const std::string& filterStr)
{
    intptr_t hFile = 0;
        ...
        do {
            if ((fileinfo.attrib & _A_SUBDIR)) {
                if (strcmp(fileinfo.name, ".") != 0 && strcmp(fileinfo.name, "..") != 0)
                    Util::GetFiles(p.assign(path).append("/").append(fileinfo.name), files, filterStr);
            } else {
                if (filterStr != "") {
                    if (std::string(fileinfo.name).find(filterStr) != std::string::npos) {
...
std::string Util::UnicodeToANSI(const std::wstring& wstr)
{
...
void Util::ReplaceToESC(std::string& str)
{
    Replace(str, "\\0", "\0", 1);
    Replace(str, "\\n", "\n", 1);
    Replace(str, "\\r", "\r", 1);
...
bool Util::IsDirExist(const std::string& fullPath)
{
#ifdef _WINDOWS
    if (-1 == _access(fullPath.c_str(), 0))
        return false;
...
std::string Util::GetFileNameWithOutExt(const std::string& fullPath)
{
...
std::string Util::GetFilePath(const std::string& fullPath)
{
    std::string path = fullPath;
    std::string na = fullPath;
    Replace(path, "\\", "/", 1);
...
int Util::ExecCommand(const std::string& cmd, std::string& output)
{
...
void Util::Trim(std::string& src, const char* chs, size_t len)
{
    if (chs == NULL) {
        return;
    }
...
void Util::KillThread(std::thread* thread)
{
#ifdef _WINDOWS
...
std::string Util::GetIconFullPathName(const std::string& icon)
{
    std::string fullpathname = Util::GetWorkDirectory() + "/" + icon;
    if (Util::IsFileExist(fullpathname))
        fullpathname = "";
    return fullpathname;
}

bool Util::StrCompare(const std::string& str1, const std::string& str2)
{
    std::string s1, s2;
    s1 = str1;
    s2 = str2;
...
std::string Util::strToHex(const std::string& convert_data)
{
    std::string outStr;
...
std::string Util::hexToStr(const std::string& convert_data)
{
    std::string outStr;
    char buf[6];
...
void Util::LaunchThirdProgram(QProcess* const process, const QString& name, const QString& args)
{
    QString appDir = QApplication::applicationDirPath();
#ifdef Q_OS_MAC
    QString cmd = "open " + appDir.section("/", 0, -4) + "/" + name + ".app";
#else
    QString cmd = appDir + "/" + name;
#endif
